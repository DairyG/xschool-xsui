layui.define(function(exports){
	...
	var pluginName = 'pickArea',area = areaJson.data,picknum = 1;
	var pick_pcc = {
		render: function (param) {
			...
			this.config = $.extend({},
			defaults, param);
			this.ele = $(this.config.elem);
			console.log(this.ele);
			this.config.format = this.config.format ? this.config.format.replace(/,/g,'/') : this.config.format;
			this.init();
			return this;
		},		
		init: function() {
			...
			if (format && format != "") {
				...
				contentStr = '<div class="pick-show">' + '<input type="hidden" value="" data-areacode="" class="pick-area" readonly/>' + '<span class="pick-province">请选择省</span><i>/</i>' + '<span class="pick-city">请选择市</span><i>/</i>' + '<span class="pick-county">请选择县</span>' + '<em class="pick-arrow"></em>' + '</div>' + '<ul class="pick-list">' + '</ul>';
				this.format = "p/c/c";
			}
			if (setFormat) {
				var slash = 0,
				...
				if (setFormat.match("province")) {
					...
					contentStr = '<div class="pick-show">' + '<input type="hidden" value="" class="pick-area" data-areacode="" readonly/>' + strSpan + '<em class="pick-arrow"></em>' + '</div>' + '<ul class="pick-list">' + '</ul>';
				} else {
					if (slash == 0) {
					...
					contentStr = '<div class="pick-show">' + '<input type="hidden" value="' + setFormat + '" class="pick-area" data-areacode="" readonly/>' + strSpan + '<em class="pick-arrow"></em>' + '</div>' + '<ul class="pick-list">' + '</ul>';
				}
			}
			...
			if (s == "pro") {
				...
				if ($(aim).html() == "请选择省") {
					...
					if (($(aim).html() == "台湾省" || $(aim).html() == "香港特别行政区" || $(aim).html() == "澳门特别行政区") && this.format != "p") {
						...
						this.ele.find(".pick-area-hidden").val(this.ele.find(".pick-province").html());
						return;
					} else {
				...
				if ($(aim).html() == "请选择市") {
					...
					$(".pick-list").scrollTop(0);
					this.ele.find(".pick-area-hidden").val(this.ele.find(".pick-province").html() + " " + this.ele.find(".pick-city").html());
				...
				if ($(aim).html() == "请选择县") {
					...
					$(".pick-list").scrollTop(0);
					this.ele.find(".pick-area-hidden").val(this.ele.find(".pick-province").html() + " " + this.ele.find(".pick-city").html() + " " + this.ele.find(".pick-county").html());
		...
		setBack: function(_this) {
			...
			if (typeof _this.config.getVal === "function") {
				let name = _this.ele.find('input[type=hidden].pick-area').val();
				name = name.replace(/\//g,',');
				let code = _this.ele.find('input[type=hidden].pick-area').attr('data-areacode');
				_this.config.getVal(name,code);
			}
		},
		getval:function(){
			let name = this.ele.find('input[type=hidden].pick-area').val();
			name = name.replace(/\//g,',');
			let code = this.ele.find('input[type=hidden].pick-area').attr('data-areacode');
			return {name,code};
		},
	...
	layui.link(layui.cache.base + 'pick-pcc/pick-pcc.css');
	exports('pick_pcc', pick_pcc);
});  